...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($username) || empty($email) || empty($password) || empty($confirm_pass) || empty($first_name) || empty($last_name1) || empty($last_name2) || $password != $confirm_pass) {
    ...
    if ($stmt) {
      $stmt->bind_param("ssssssss", $username, $email, $password_hash, $address, $card_number, $first_name, $last_name1, $last_name2);
      if ($stmt->execute()) {
        header("Location: iniciarsesion.php?registration=success");
        exit();
      } else {
